...
function setupScoreDetails(d,c){
 ...
 const open=k=>{const [title,w,desc]=meta[k], rows=scoreDetailRows(k,d,c), el=document.getElementById('scoreExplanation');setText('scoreExplainTitle',title+' 점수 산정 근거');setText('scoreExplainWeight','매수여건 지수 가중치 '+w);document.getElementById('scoreExplainBody').innerHTML='<p class="score-rule">'+esc(desc)+'</p><div class="score-detail-rows">'+rows.map(r=>'<div><span>'+esc(r[0])+'</span><b>'+esc(r[1])+'</b><small>'+esc(r[2])+'</small></div>').join('')+'</div><p class="score-note">미연결 데이터는 50점으로 임의 대체하지 않고 전체 신뢰도에서 제외합니다.</p>';el.hidden=false};
