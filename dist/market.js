'use strict';const eok=n=>n==null?'—':(Number(n)/10000).toLocaleString('ko-KR',{maximumFractionDigits:2})+'억';async function renderKbHistory(){
 ...
 try{
  ...
  axis.innerHTML=a.map(x=>'<span>'+x.d.slice(2,4)+'.'+x.d.slice(4,6)+'</span>').join('');
  if(note)note.textContent='KB 관심단지 월별 중앙값 · '+a[0].d+' ~ '+a[a.length-1].d+' · 최신 표본 '+a[a.length-1].n+'개';
 }catch(e){
...
async function loadMarketIndicators(){
 try{
  ...
  sourceRefreshNote=delayed.length?' · 원천 지연 '+delayed.join('/'):' · 원천 갱신 확인';
