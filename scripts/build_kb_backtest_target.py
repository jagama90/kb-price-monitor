...
S=R/'dist/garak_geumho_24a_history.json'
O=R/'dist/kb_price_backtest_target.json'
def main():
 raw=S.read_text().strip() if S.exists() else ''
 try:
  d=json.loads(raw) if raw else {}
 except json.JSONDecodeError:
  d={}
 rows=[x for x in d.get('series',[]) if x.get('ym') and x.get('sale')]
