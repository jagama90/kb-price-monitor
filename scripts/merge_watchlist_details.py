...
kb_exact={(int(x['complex_id']),int(t['area_id'])):t.get('general_price_manwon') for x in master.get('items',[]) if x.get('complex_id') for t in x.get('types',[]) if t.get('area_id')}
...
for row in market.get('items',[]):
    ...
    exact=kb_exact.get((int(row.get('complex_id') or 0),int(row.get('area_id') or 0)))
    if exact is not None: row['kb_general_check_manwon']=exact
    matched+=1
